...
function nowInTunis() {
  ...
  return { todayISO: `${yyyy}-${mm}-${dd}`, nowHH: hh, nowMM: mi, nowKey: `${yyyy}-${mm}-${dd}T${hh}:${mi}:00` };
}
...
export default function ScheduleSeancesForDossierModal({
  ...
  const minDate = useMemo(() => {
    ...
    const c = [todayISO, lr, lastScheduledDate].filter(Boolean) as string[];
    return c.length ? c.sort().at(-1)! : todayISO;
  ...
  useEffect(() => {
    (async () => {
      if (isAdmin) {
        const { data } = await supabase.from('users_base')
    ...
    setRows((prev) =>
      prev.map((r, i) => {
        ...
        if (minHHMM) {
          ...
          const minKey = keyFrom(newDate, mh, mm);
          if (parseKey(curKey) < parseKey(minKey)) return { ...r, date: newDate, hour: mh, minute: mm };
        }
  ...
  useEffect(() => {
    ...
    let last = gen[0]; let lastDateObj = new Date(gen[0].date);
  ...
  /* --------- edition --------- */
  const canSchedule = (dossier.etat === 'en_cours' || dossier.etat === 'a_venir') && totalPrevues > 0 && remaining > 0;
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* panneau : hauteur bornée + colonne */}
      <div className="absolute inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-[92vw] md:max-w-3xl">
        <div className="relative bg-white rounded-none md:rounded-xl shadow-xl flex flex-col max-h-[90vh]">
          {/* Header */}
          ...
          {/* CONTENU SCROLLABLE — espace réservé pour le footer pour éviter tout chevauchement */}
              ...
              <div className="flex items-center justify-between">
                ...
                {copyEnabled && (
                  <span className="hidden sm:inline-flex items-center gap-2 text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded">
                    <Copy className="w-3 h-3" /> Aperçu plus bas
                  </span>
                )}
              </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="inline-flex items-center justify-center rounded-lg bg-gray-100 px-2 py-2 text-xs text-gray-600">tous les</span>
                        <input
                          type="number"
                          min={1}
                          value={stepDays}
                    ...
                    <div className="hidden lg:flex items-end">
                      <div className="text-xs text-gray-600 inline-flex items-center gap-2 px-2 py-2 bg-gray-50 rounded w-full">
                        <Clock className="w-3 h-3" /> Bornes auto (heure de Tunis), aucune superposition.
                      </div>
                    </div>
                  </div>

                  {/* APERÇU : conteneur scrollable pour éviter tout dépassement */}
                    ...
                    <div className="max-h-56 overflow-auto">
                      <table className="min-w-full text-sm">
                        ...
                        <tbody>
                          ...
                          {rows.length === 0 && (
                            <tr><td colSpan={4} className="px-3 py-3 text-center text-gray-500">Aucun créneau.</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
          ...
          {/* FOOTER collant : ombre + hauteur explicite */}
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
      ...
      <div className="sm:col-span-1 flex items-end">
        {canRemove && onRemove && (
          <button type="button" onClick={onRemove} className="inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg border hover:bg-gray-50 w-full sm:w-auto" title={`Supprimer la séance ${numero}`}>
